...
class Assets:
    ...
    CHECKMARK: Path = DIRECTORY / "checkmark.png"
...
class Colors:
    class Dark:
        ...
        class BorderActiveHoverColor:
            hex: str = get_ActiveHoverColor(winaccent.accent_light_2, darkmode=True, border=True)
            r: int = int(hex[1:3], 16)
            g: int = int(hex[3:5], 16)
            b: int = int(hex[5:7], 16)
    ...
    class Light:
        ...
        class BorderActiveColor:
            ...
            b: int = int(hex[5:7], 16)
        class BorderActiveHoverColor:
            hex: str = get_ActiveHoverColor(winaccent.accent_dark_1, border=True)
            r: int = int(hex[1:3], 16)
            g: int = int(hex[3:5], 16)
            b: int = int(hex[5:7], 16)
        class BorderActiveClickedColor:
...
class FluentCheckBox(ctk.CTkLabel):
    value: bool = False
    hovered: bool = False
    clicked: bool = False
    ...
    def _create_image_objects(self) -> None:
        ...
        with Image.open(Assets.CHECKMARK) as checkmark:
            checkmark_data = checkmark.getdata()
            checkmark.putdata([(Colors.Dark.CheckMarkColor.r, Colors.Dark.CheckMarkColor.g, Colors.Dark.CheckMarkColor.b, a) for _, _, _, a in checkmark_data])
            self._dark_image_active = Image.alpha_composite(self._dark_image_active, checkmark)
            self._dark_image_active_hover = Image.alpha_composite(self._dark_image_active_hover, checkmark)
            self._dark_image_active_clicked = Image.alpha_composite(self._dark_image_active_clicked, checkmark)

            checkmark.putdata([(Colors.Light.CheckMarkColor.r, Colors.Light.CheckMarkColor.g, Colors.Light.CheckMarkColor.b, a) for _, _, _, a in checkmark_data])
            self._light_image_active = Image.alpha_composite(self._light_image_active, checkmark)
            self._light_image_active_hover = Image.alpha_composite(self._light_image_active_hover, checkmark)
            self._light_image_active_clicked = Image.alpha_composite(self._light_image_active_clicked, checkmark)


    def _finalize_image_objects(self) -> None:
        self.image_default = ctk.CTkImage(light_image=self._light_image_default, dark_image=self._dark_image_default, size=(20, 20))
        self.image_hover = ctk.CTkImage(light_image=self._light_image_hover, dark_image=self._dark_image_hover, size=(20, 20))
        self.image_clicked = ctk.CTkImage(light_image=self._light_image_clicked, dark_image=self._dark_image_clicked, size=(20, 20))
        self.image_active = ctk.CTkImage(light_image=self._light_image_active, dark_image=self._dark_image_active, size=(20, 20))
        self.image_active_hover = ctk.CTkImage(light_image=self._light_image_active_hover, dark_image=self._dark_image_active_hover, size=(20, 20))
        self.image_active_clicked = ctk.CTkImage(light_image=self._light_image_active_clicked, dark_image=self._dark_image_active_clicked, size=(20, 20))