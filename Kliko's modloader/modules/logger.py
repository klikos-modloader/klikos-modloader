...
class Logger:
    ...
    def initialize(cls) -> None:
        ...
        cls.filename = f"{cls.timestamp}_{uuid.uuid4().hex[:5].upper()}.txt"
        ...
        for log in DIRECTORY.iterdir():
            ...
            if age > timedelta(days=MAX_FILE_AGE):
                try:
                    ...
                    cls.warning(f"Unable to remove log file: {log.name}! {e}", prefix="Logger.cleanup()")


    @classmethod
    def write(cls, level: int, message: object, prefix: Optional[str] = None) -> None:
        if not cls._initialized: raise RuntimeError(f"Logger was not initialized")
...
Logger.initialize()  # Initialize on import